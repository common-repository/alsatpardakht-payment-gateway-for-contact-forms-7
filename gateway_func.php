...
function alsatpardakht_result_payment_func()
{
    ...
    if (isset($_GET['iN'])) {
        ...
        if (isset($iN) && isset($iD)) {
            ...
            if ($vasetIGP) {
                // verify payment from alsatpardakht gateway
...
register_uninstall_hook(__FILE__, "alsatpardakht_cf7pp_uninstall");
...
function alsatpardakht_cf7pp_deactivate()
{
    ...
    delete_option("alsatpardakht_cf7pp_my_plugin_notice_shown");
}


function alsatpardakht_cf7pp_uninstall()
{
}

// display activation notice
add_action('admin_notices', 'alsatpardakht_cf7pp_my_plugin_admin_notices');

function alsatpardakht_cf7pp_my_plugin_admin_notices()
{
    if (!get_option('alsatpardakht_cf7pp_my_plugin_notice_shown')) {
        echo wp_kses_post("<div class='updated'><p><a href='admin.php?page=alsatpardakht_cf7pp_admin_table'>برای تنظیم اطلاعات درگاه  کلیک کنید</a>.</p></div>");
        update_option("alsatpardakht_cf7pp_my_plugin_notice_shown", "true");
    }
}
...
if (is_plugin_active('contact-form-7/wp-contact-form-7.php')) {
    ...
    add_action('admin_menu', 'alsatpardakht_cf7pp_admin_menu', 20);
    function alsatpardakht_cf7pp_admin_menu()
    {
        $addnew = add_submenu_page('wpcf7',
            __('تنظیمات آل‌سات پرداخت', 'contact-form-7'),
            __('تنظیمات آل‌سات پرداخت', 'contact-form-7'),
            'wpcf7_edit_contact_forms', 'alsatpardakht_cf7pp_admin_table',
            'alsatpardakht_cf7pp_admin_table');

        $addnew = add_submenu_page('wpcf7',
            __('لیست تراکنش ها', 'contact-form-7'),
            __('لیست تراکنش ها', 'contact-form-7'),
            'wpcf7_edit_contact_forms', 'alsatpardakht_cf7pp_admin_list_trans',
            'alsatpardakht_cf7pp_admin_list_trans');

    }


    // hook into contact form 7 - before send
    add_action('wpcf7_before_send_mail', 'alsatpardakht_cf7pp_before_send_mail');
    function alsatpardakht_cf7pp_before_send_mail($cf7)
    {
    }


    // hook into contact form 7 - after send
    add_action('wpcf7_mail_sent', 'alsatpardakht_cf7pp_after_send_mail');
    function alsatpardakht_cf7pp_after_send_mail($cf7)
    {
        global $wpdb;
        global $postid;
        $postid = $cf7->id();
    ...
    add_action('wpcf7_admin_after_additional_settings', 'alsatpardakht_cf7pp_admin_after_additional_settings');
    function alsatpardakht_cf7pp_editor_panels($panels)
    {

        $new_page = array(
            'PricePay' => array(
                'title' => __('اطلاعات پرداخت', 'contact-form-7'),
                'callback' => 'alsatpardakht_cf7pp_admin_after_additional_settings'
            )
        );
    ...
    add_filter('wpcf7_editor_panels', 'alsatpardakht_cf7pp_editor_panels');


    function alsatpardakht_cf7pp_admin_after_additional_settings($cf7)
    {
    ...
    add_action('wpcf7_save_contact_form', 'alsatpardakht_cf7pp_save_contact_form');
    function alsatpardakht_cf7pp_save_contact_form($cf7)
    {
    ...
    function alsatpardakht_cf7pp_admin_list_trans()
    {
                ...
                <center>
                    <div class="tablenav">
                        <div class="tablenav-pages"
                             style="float:none; margin: 1em 0"><?php echo wp_kses_post($page_links) ?></div>
                    </div>
                </center>
                <?php
            }
    ...
    function alsatpardakht_cf7pp_admin_table()
    {
        global $wpdb;
        if (!current_user_can("manage_options")) {
            wp_die(__("You do not have sufficient permissions to access this page."));
        ...
        ?>
        <form method="post" action='<?php echo esc_html($_SERVER["REQUEST_URI"]) ?>' enctype="multipart/form-data">
        <?php
...
<br/>
 ...
 green
    ...
    function alsatpardakht_cf7pp_my_admin_notice()
    {
    ...
    add_action('admin_notices', 'alsatpardakht_cf7pp_my_admin_notice');
}
?>